...
class TestMushTools < Test::Unit::TestCase
  def test_simple
    ...
    rot1.mRotate(vec1)
    assert(vec1.mApproxEqual(Mush4Val.new(0,1,0,0), 1e-6))
    rot1 = MushTools.sRotationInYZPlane(Math::PI/2)
    rot1.mRotate(vec1)
    assert(vec1.mApproxEqual(Mush4Val.new(0,0,1,0), 1e-6))
    rot1 = MushTools.sRotationInZWPlane(Math::PI/2)
    rot1.mRotate(vec1)
    assert(vec1.mApproxEqual(Mush4Val.new(0,0,0,1), 1e-6))
    rot1 = MushTools.sRotationInXWPlane(-Math::PI/2)
    rot1.mRotate(vec1)
    assert(vec1.mApproxEqual(Mush4Val.new(1,0,0,0), 1e-6))
